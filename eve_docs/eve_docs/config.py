...
def schema(resource):
    ret = []
    keys = resource["schema"].keys()
    for k,v in resource["schema"].items() :
        dict = {}
        dict["name"] = k
        dict["type"] = v["type"] if "type" in v.keys() else None
        dict["required"] = v["required"] if "required" in v.keys() else False
'''
